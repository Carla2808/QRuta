...
| 16 | **Perfil del usuario** | Opcional: yo como posible víctima. | Vacío: qué es, 3 garantías (privado, 2 min, reversible), "Crear mi perfil". Creación en 4 pasos: **teléfono + OTP** → datos (nombre, sangre, año, alergias, condiciones, medicación, notas) → contactos (máx. 3, con canal) → **consentimiento con tabla "quién ve qué"**. Creado: grilla médica, contactos, **tarjeta SALVO con QR personal**, "quién vio mis datos", Editar / Borrar. | "Crear mi perfil" → "Crear perfil" | "Editar" · "Borrar perfil" · "Ver cómo la ve otra persona" | OTP incorrecto: vibración + "Código incorrecto"; sin nombre: no avanza; sin contactos: no avanza; sin consentimiento: botón deshabilitado. Sin red: se guarda local y sincroniza. Persistencia en el teléfono (demo) / servidor cifrado (producción). | Campos con `label` e `id` estables, `inputmode` numérico/tel, `autocomplete`, foco no se pierde al escribir. |
| 17 | **Contactos** | Quién recibe aviso si me pasa algo. | Lista con nombre, relación, número enmascarado, canal, estado de verificación; alta dentro del flujo de perfil. | "+ Agregar contacto" | "Quitar" | Máx. 3; contacto sin verificar: pill ámbar; el aviso al contacto nunca incluye datos médicos. | Botón "Quitar" con `aria-label` que nombra al contacto. |
| 16b | **Tarjeta SALVO personal / Persona identificada** | El perfil como QR físico (billetera, casco). | Tarjeta "A SALVO" con QR `?p=SCZ-P-XXXX`, nombre + inicial, código, sangre. Quien la escanea ve: banda verde "Persona con perfil SALVO", nombre + inicial, grilla médica **borrosa** (salvo servicio acreditado), "Avisar a sus contactos", "Llamar a emergencias", "Soy personal de emergencia". | "Avisar a sus contactos" | Llamar · elevar acceso | Código desconocido: aviso + llamar. Elevación registra acceso en la auditoría del titular. | Misma gramática que la ficha del vehículo: aprendida una vez, sirve para las dos. |
