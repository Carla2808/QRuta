...
self.addEventListener('fetch', e => {
  ...
  if (url.pathname.includes('/v1/')) {
    e.respondWith(fetch(e.request).then(r => { const copy = r.clone(); caches.open('salvo-api').then(c => c.put(e.request, copy)); return r; }).catch(() => caches.match(e.request)));
    return;
  }
